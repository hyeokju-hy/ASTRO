...
def test_torch(ua_embeddings, ia_embeddings, users_to_test, is_val, adj, beta, target_aware):
    ...
    for u_batch_id in tqdm(range(n_user_batchs), desc='evaluating'):
        ...
        if target_aware:
            ...
            for i_batch_id in range(n_item_batchs):
                ...
                item_target_user_alpha = torch.softmax(
                    torch.multiply(item_query.unsqueeze(1), adj[user_batch, :].unsqueeze(0)).masked_fill(
                        adj[user_batch, :].repeat(len(item_batch), 1, 1) == 0, -1e9), dim=2)
                item_target_user = torch.matmul(item_target_user_alpha, ia_embeddings)

                # user side target-aware
                i_rate_batch = (1 - beta) * torch.matmul(u_g_embeddings, torch.transpose(i_g_embeddings, 0, 1)) + \
                               beta * torch.sum(torch.mul(item_target_user.permute(1, 0, 2).contiguous(), i_g_embeddings), dim=2)

                rate_batch[:, i_start: i_end] = i_rate_batch.detach().cpu().numpy()
                i_count += i_rate_batch.shape[1]

                del item_query, item_target_user_alpha, item_target_user, i_g_embeddings, u_g_embeddings
        ...
        for re in map(test_one_user, user_batch_rating_uid):
